...
export function KanjiDrawer({ word, isOpen, onOpenChange, initialKanji }: KanjiDrawerProps) {
    ...
    const content = (
        <div className="flex-1 flex flex-col min-h-0 pt-2 px-4">
            <Tabs value={selectedKanji} onValueChange={setSelectedKanji} className="flex-1 flex flex-col">
                {kanjis.length > 1 && (
                    <TabsList className="w-full shrink-0 flex flex-nowrap overflow-x-auto justify-start mb-4 bg-muted/50 p-1">
                        {kanjis.map((k) => (
                            <TabsTrigger key={k} value={k} className="flex-1 sm:flex-none py-2 text-lg font-bold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                                {k}
                            </TabsTrigger>
                        ))}
                    </TabsList>
                ...
                <div className="flex-1 relative pb-0">
                    {kanjis.map((k) => (
                        <TabsContent key={k} value={k} className="absolute inset-0 m-0 data-[state=inactive]:hidden">
                                                    ...
                                                    <div className="md:col-span-9 lg:col-span-10 flex flex-col justify-center space-y-6 md:pl-4">
                                                        <div>
                                                            <div className="flex items-center gap-3 mb-2">
                                                                ...
                                                                {details?.hanviet && <Badge variant="secondary" className="text-xs px-2 py-0.5 font-semibold uppercase tracking-wider">{details.hanviet}</Badge>}
                                                            </div>
                                                            <p className="text-xl font-medium text-foreground">{details?.meaning || "—"}</p>
                                                        </div>
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                            <div className="bg-muted/30 p-4 rounded-md">
                                                                ...
                                                                <p className="font-japanese text-xl">{details?.onyomi || "—"}</p>
                                                            </div>
                                                            <div className="bg-muted/30 p-4 rounded-md">
                                                                ...
                                                                <p className="font-japanese text-xl">{details?.kunyomi || "—"}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    <div className="space-y-6">
                                                        {details?.hint && (
                                                            <div>
                                                                ...
                                                                <div className="prose prose-sm dark:prose-invert max-w-none text-muted-foreground bg-primary/5 p-4 rounded-lg border border-primary/10 [&_img]:bg-white [&_img]:rounded-md [&_img]:p-0.5 [&_img]:inline-block [&_img]:w-6 [&_img]:h-6 [&_img]:align-text-bottom [&_img]:ml-1" dangerouslySetInnerHTML={{ __html: details.hint }} />
                                                            </div>
                                                        )}
                                                        {details?.radicals && details.radicals.length > 0 && (
                                                            <div>
                                                                ...
                                                                <div className="flex flex-wrap gap-2">
                                                                    {details.radicals.map((rad: any, i: number) => (
                                                    ...
                                                    <div className="space-y-6">
                                                        <Tabs defaultValue="vn" className="w-full">
                                                            ...
                                                            <TabsContent value="vn" className="mt-4 space-y-3">
                                                                {details?.examples?.vn?.map((ex: any, i: number) => (
                                                                    <div key={i} className="flex flex-col p-3 rounded-md bg-muted/40 hover:bg-muted/70 transition-colors border border-transparent hover:border-border">
                                                                        <div className="flex items-baseline gap-2 mb-1">
                                                                            <span className="font-bold text-lg text-primary font-japanese">{ex.word}</span>
