  ...
  return (
    <Card ref={containerRef} className="relative h-full flex flex-col hover:shadow-lg hover:scale-105 transition-all duration-300">
      <CardHeader>
        <div>
          {editing ? (
            <input className="text-xl font-headline w-full bg-transparent outline-none" value={title} onChange={(e) => setTitle(e.target.value)} />
          ) : (
            <CardTitle className="font-headline cursor-text" onClick={() => setEditing(true)}>{topic.title}</CardTitle>
          )}
          {editing ? (
            <textarea className="text-sm text-muted-foreground w-full mt-1 bg-transparent outline-none" value={description} onChange={(e) => setDescription(e.target.value)} />
          ) : (
            <CardDescription onClick={() => setEditing(true)}>{topic.description}</CardDescription>
          )}
        </div>
      </CardHeader>

      <CardContent>
        <p className="text-sm text-muted-foreground">{topic.vocabularyCount} word(s)</p>
      </CardContent>
      ...
      <div className="absolute top-2 right-2 z-20 flex items-center">
        <AlertDialog>
          ...
          <AlertDialogContent>
            ...
            <p>Are you sure you want to permanently delete the topic "{topic.title}" and its vocabulary? This action cannot be undone.</p>
      ...
      {!editing && <Link href={`/projects/${projectId}/topics/${topic.id}`} className="absolute inset-0 z-10" aria-hidden />}
    </Card>
  );
}
